...
- 欧几里得算法：如何求解两个大整数的gcd呢？假设大的那个数字是a，小的那个数字是b
...
- 贝祖（裴蜀）定理：如果a和b是**正整数**，必定存在**整数**s和t，使得$gcd(a, b) = sa+tb$。证明请看：[oi-wiki 裴蜀定理](https://oi-wiki.org/math/number-theory/bezouts/)
    ...
    > $...$
    >
    > $m_{n-2}=r_{n}m_{n-1} + m_{n}$
    >
    > $m_{n-1} = r_{n+1} m_{n}$，直到找到余数为0，那么最大公约数就是$m_n$
    >
    > 从倒数第二步反着带进去，有
    >
    > $m_n = m_{n-2} - r_{n}m_{n-1}$
    >
    > $m_{n} = m_{n-2} - r_n(m_{n-3} - r_{n-1}m_{n-2})$拆开以后，继续带入
    >
    > …
    >
    > 直到a，b被带入即可
...
&emsp;形如$ax\equiv b(\mod m)$的线性方程称之为同余方程。其中，m是正整数，a，b是整数，x是变量。怎么求解线性同余方程呢？即，如何找到所有满足该方程的整数x。接下来介绍一个方法是：$\overline a a\equiv 1(\mod m)$，如果这样的整数存在，称$\overline a$为a模m的逆。
...
- 一个正合数n如果对于所有满足$gcd(b, n)=1$的正整数b都有同余式$b^{n-1} \equiv 1(\mod n)$成立，则称为卡米切尔数。
